...
Widget _buildTestApp(Stream<EventModel?> eventStream, SharedPreferences prefs) {
  ...
  return ProviderScope(
    overrides: [
      ...
      registrationsProvider
          .overrideWith((ref, arg) => Stream.value(<RegistrationModel>[])),
      teamsProvider
          .overrideWith((ref, arg) => Stream.value(<TeamModel>[])),
      liveTrackingProvider
          .overrideWith((ref, arg) => Stream.value(<GpsPointModel>[])),
      passagesStreamProvider
          .overrideWith((ref, arg) => Stream.value(<WaypointPassageRecord>[])),
      withdrawalsStreamProvider
          .overrideWith((ref, arg) => Stream.value(<String>{})),
      currentUserModelProvider.overrideWith((_) async => null),
      myRegistrationStreamProvider
          .overrideWith((ref, arg) => Stream.value(null)),
    ],
...
void main() {
  ...
  group('EventManagementScreen', () {
    testWidgets('shows loading indicator while stream is loading',
        (tester) async {
      await tester.pumpWidget(_buildTestApp(const Stream.empty(), prefs));
      await tester.pump();
    ...
    testWidgets('shows event name in AppBar when event is loaded',
        (tester) async {
      await tester.pumpWidget(
          _buildTestApp(Stream.value(_makeEvent()), prefs));
      await tester.pump();
    ...
    testWidgets('shows APERTO status label for aperto event', (tester) async {
      await tester.pumpWidget(
          _buildTestApp(Stream.value(_makeEvent(stato: EventStatus.aperto)), prefs));
      await tester.pump();
    ...
    testWidgets('shows IN CORSO status label for inCorso event',
        (tester) async {
      await tester.pumpWidget(
          _buildTestApp(Stream.value(_makeEvent(stato: EventStatus.inCorso)), prefs));
      await tester.pump();
    ...
    testWidgets('shows CONCLUSO status label for concluso event',
        (tester) async {
      await tester.pumpWidget(
          _buildTestApp(Stream.value(_makeEvent(stato: EventStatus.concluso)), prefs));
      await tester.pump();
    ...
    testWidgets('lock icons appear on tabs 1-4 when event is bozza',
        (tester) async {
      await tester.pumpWidget(
          _buildTestApp(Stream.value(_makeEvent(stato: EventStatus.bozza)), prefs));
      await tester.pump();
  ...
  group('_statusLabel logic', () {
    test('all status values map to correct labels', () {
      const values = {
