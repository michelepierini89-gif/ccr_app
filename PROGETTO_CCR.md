...
│   │   │   │   ├── create_event_screen.dart    # con dimensione squadra + tipologia
...
│   │   │   │   └── specials_editor_screen.dart
│   │   │   └── widgets/ (event_card_admin, special_tile)
│   │   │
│   │   ├── pilot/
...
- `EventManagementScreen`: tab Tracciato / Iscrizioni / Live — tab bloccate in BOZZA, layout responsive (wide ≥600px, mobile verticale)
- Upload GPX/KML tramite FilePicker → Storage → Firestore (`trackUrl`)
- Auto-caricamento tracciato da Storage all'apertura
...
- `SpecialsEditorScreen`: editor prove speciali con waypoint su mappa
...
- `EventDetailScreen`: header evento, mappa tracciato (caricamento da Storage), prove speciali, stato iscrizione, link squadra
- `TeamScreen`: crea/unisciti/lascia squadra, nomi membri risolti da registrazioni
...
### Fix/Ottimizzazioni sessione 03/06/2026
- **Anti-flickering**: `EventManagementScreen` e `EventDetailScreen` ora usano `eventStreamProvider` (stream su singolo documento Firestore) invece di stream sull'intera collection
- **Track pilota**: `EventDetailScreen` carica e mostra il tracciato GPX/KML da Storage
- **Nome evento GPS**: `GpsRecordingScreen` mostra il nome dell'evento invece dell'ID troncato
- **Waypoint reali**: `GpsRecordingScreen.startRecording()` passa i waypoint estratti dagli speciali dell'evento
- **Nomi membri**: `TeamScreen` mostra nome/cognome dei membri invece degli UID grezzi
- **CORS Storage**: creati `cors.json` e `set-storage-cors.js` (vedi prossimi passi)
...
| Flickering schermate evento | Stream su collection intera → rebuild ad ogni evento | Aggiunto `getEventById()` stream su singolo doc + `eventStreamProvider` |
| Track non visibile lato pilota | `EventDetailScreen` passava `trackPoints: const []` | Ora carica il tracciato da Storage come admin |
| Waypoint GPS vuoti | `startRecording()` riceveva `waypoints: const []` | Ora carica i waypoint dagli speciali dell'evento |

---

## Prossimi Step

### CORS Firebase Storage — DA COMPLETARE
Errore: `Failed to fetch` su `firebasestorage.googleapis.com` (CORS bloccato su web).

**Opzione A — Google Cloud Shell (più semplice, nessuna installazione):**
1. Apri [console.cloud.google.com](https://console.cloud.google.com) → progetto `ccr-enduro`
2. Clicca l'icona della Shell in alto a destra (>_)
...
**Opzione B — Script Node.js locale:**
...
- [ ] CORS Firebase Storage (vedi sopra)
